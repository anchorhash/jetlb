...
lru_size_to_connection_target_ratio_list = [i/100 for i in range(1, 15, 2)]
        ...
        for update_rate in update_rate_list:
            for lru_size_to_connection_target_ratio in lru_size_to_connection_target_ratio_list:
                for num_horizon in num_horizon_list:
                    ...
                    command += ' --file_name {}_fig_4b'.format(ch_type)

                    with open('./results/'
                              + '{}_fig_4b.logger.txt'.format(ch_type), 'a+') as filehandle:
