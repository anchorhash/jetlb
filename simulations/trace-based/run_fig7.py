...
if __name__ == '__main__':
	...
	### trials (i.e., seeds)
	parser.add_argument('--trials', '-t', default=10, type=int, help='number of different seeds')     

	### trace length (100M at most)
	parser.add_argument('--trace_length', '-tl', default=10**8, type=int, help='trace length')  
	...
	for random_trial in range(args.trials):
		...
		total_sim = len(hash_types) * len(traces_paths) * len(trace_length) * len(num_servers) * 2
		
		finished_sim = 0
		for hash_type in hash_types:
			for trace_path, skew in zip(traces_paths, traces):
				for ns in num_servers:
					for tr in trace_length:
						for horizon in [0, int(0.1*ns)]:
							...
							command += ' >> zipf_results_{}.txt'.format(random_trial)
							
							python_args = 'zipf_skew:{} trace_len:{} num_servers:{} horizon:{} hash_type:{}'.format(skew, tr, ns, horizon, hash_type)
							os.system('echo ' + python_args + ' >> zipf_results_{}.txt'.format(random_trial))
